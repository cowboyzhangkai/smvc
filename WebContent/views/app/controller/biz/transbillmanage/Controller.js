Ext
		.define(
				'app.controller.biz.transbillmanage.Controller',
				{
					...
					views : [ 'app.view.biz.transbillmanage.List',
							'app.view.biz.transbillmanage.add.Add',
							'app.view.biz.transbillmanage.edit.Edit' ],
					...
					init : function() {
						...
						// 添加分页监听事件
						Ext.getCmp('transbillManagerPagingToolbarId').on(
								"beforechange", this.onBeforechange, this);
						this.control({
							'transbillmanagelist' : {
								// 表格渲染
								'render' : this.render
					...
					search : function() {
						...
						Ext.data.StoreManager.lookup(
								"app.store.biz.transbillmanage.Store").load({
							params : {
								start : 0,
								limit : limit,
								billNo : billNo
						...
						for (var i = 0; i < rsArray.length; i++) {
							if (grid.getStore().indexOf(rsArray[i]) == 0) {
								if (currentpage != 1) {
									...
									grid.getDockedItems('toolbar[dock=bottom]')[0]
											.getStore().currentPage--;
								}
							}
						}
					 ...
					 */
					updateB : function() {
						var thisgrid = Ext.ComponentQuery
								.query("transbillmanagelist")[0];
					 ...
					 */
					addB : function() {
						Ext.ComponentQuery.query("addtransbillform")[0]
								.getForm()
								.submit(
										{ // 进行AJAX请求
											waitMsg : "数据保存中...",
					...
					add : function() {
						...
						if (!Ext.ComponentQuery.query("addtransbillwindow")[0]) {
							Ext.create(
									'app.view.biz.transbillmanage.add.Window',
									{}).show();
						} else {
							Ext.ComponentQuery.query("addtransbillwindow")[0]
									.show();
						}
					},
					/**
					...
					update : function() {
						...
						if (!Ext.ComponentQuery.query("edittransbillwindow")[0]) {
							Ext.create(
									"app.view.biz.transbillmanage.edit.Window",
									{}).show();
						} else if (Ext.ComponentQuery
								.query("edittransbillwindow")[0].isHidden()) {
						...
						Ext.ComponentQuery.query("edittransbillform")[0]
								.getForm().setValues([ {
									...
									id : 'fromStation',
									value : record.get('fromStation')
								}, {
									id : 'toStation',
									value : record.get('toStation')
								}, {
									id : 'transCompany',
									value : record.get('transCompany')
								}, {
									id : 'destCompany',
									value : record.get('destCompany')
								}, {
									id : 'destTel',
									value : record.get('destTel')
								}, {
									id : 'destPhone',
									value : record.get('destPhone')
								}, {
									id : 'destAddress',
									value : record.get('destAddress')
								}, {
									id : 'payType',
									value : record.get('payType')
								} ]);
