...
public class CustomFormAuthenticationFilter extends FormAuthenticationFilter {
	...
	public boolean onPreHandle(ServletRequest request,
		...
		boolean isAccessAllowed = super.onPreHandle(request, response,
				mappedValue);
		if (isAccessAllowed && isLoginRequest(request, response)
				&& SecurityHelper.isAuthed()) {
	...
	private void setUserInstanceToSession(String username) {
		SecurityUtils.getSubject().getSession()
				.setAttribute("cuser", userBus.findByName(username));
	}
